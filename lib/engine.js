...
const { REFERENCES_FOLDER_PATH, RUNS_FOLDER_PATH, FAILED_IMAGE, REPORT_FILE, PIXLJS, ERROR, SUCCESS } = require('./common');
...
class PixlJs {
  ...
  async _setup(browser) {
    ...
    page.on('console', (msg) =>
      msg.type() === 'error'
        ? console.error(`${PIXLJS}: ${ERROR('Page Error')} -`, msg.text())
        : undefined,
    );
    ...
    try {
      ...
      console.error(`${PIXLJS}: ${ERROR('Error')} - Could not navigate to scenario URL.`, error);
    }
    ...
    try {
      ...
      console.error(`${PIXLJS}: ${ERROR('Error')} - beforeEach initialise function threw error.`, error);
    }
  ...
  async runScenario(scenario) {
    ...
    for (let [index, step] of scenario.steps.entries()) {
      ...
      try {
        ...
        console.log(`${PIXLJS}: ${SUCCESS('Executed')} - ${scenario.name} - Step ${stepNumber}: '${description}'.`);
      } catch (error) {
        // Something went wrong, so mark this step as failed
        hasError = true;
        console.error(`${PIXLJS}: ${ERROR('Failed')} - ${scenario.name} - Step ${stepNumber}: '${description}'.`, error);
  ...
  approveLastRun() {
    ...
    if (!lastRun) {
      console.error(`${PIXLJS}: ${ERROR('Error')} - Could not find any visual regression run to approve.`);
      return false;
    }
      ...
      for (const step of steps) {
        ...
        for (let viewport of viewports) {
          ...
          if (!fs.existsSync(original)) {
            ...
            step.error = true;
            console.error(`${PIXLJS}: ${ERROR('Failed')} - [${viewport.name}] ${report.scenario} - Step ${step.stepNumber}: '${step.description}' has no original.`);
            ...
            if (compare.failed) {
              ...
              console.error(`${PIXLJS}: ${ERROR('Failed')} - [${viewport.name}] ${report.scenario} - Step ${step.stepNumber}: '${step.description}' is different by ${compare.misMatchPercentage}%.`);
              fs.renameSync(target, target.replace('.png', '_failed.png'));
              await fs.writeFile(target.replace('.png', '_diff.png'), compare.getDiff());
            }

            // Update return object
            step.error = step.error ? true : compare.failed;
