...
class VisualKit {
  ...
  _createFolders(scenarioFolder, timestamp) {
    // Delete old runs
    try {
      fs.readdirSync(RUNS_FOLDER_PATH)
        .sort((a, b) => b - a)
        .slice(this._options.maxRunsToKeep - 1)
        .forEach((run) => fs.removeSync(path.resolve(RUNS_FOLDER_PATH, run)));
    } catch (error) {
    }
    ...
    for (let viewport of viewports) {
      fs.mkdirSync(path.resolve(RUNS_FOLDER_PATH, timestamp, scenarioFolder, viewport.name), { recursive: true });
    }
  }
  ...
  async _takeScreenshot(scenarioFolder, timestamp, stepNumber, page) {
    ...
    for (let viewport of viewports) {
      ...
      await page.screenshot({
        path: path.resolve(RUNS_FOLDER_PATH, timestamp, scenarioFolder, viewport.name, `step${stepNumber}.png`),
        fullPage: true,
      });
    }
  }
  ...
  _failedStep(scenarioFolder, timestamp, stepNumber) {
    ...
    for (let viewport of viewports) {
      fs.copySync(
        FAILED_IMAGE,
        path.resolve(RUNS_FOLDER_PATH, timestamp, scenarioFolder, viewport.name, `step${stepNumber}_failed.png`),
      );
    }
  }
  ...
  async runScenario(scenario) {
    ...
    for (let [index, step] of scenario.steps.entries()) {
      ...
      try {
        ...
        result.steps.push({ stepNumber, description, error: null });
      } catch (error) {
  ...
  approveLastRun() {
    ...
    try {
      ...
      lastRun = fs.readdirSync(RUNS_FOLDER_PATH).sort((a, b) => b - a)[0];
    } catch (error) {
    }
    ...
    glob.sync(path.resolve(RUNS_FOLDER_PATH, lastRun) + '/**/*_failed.png')
      .map((file) => file.replace(path.resolve(RUNS_FOLDER_PATH, lastRun), ''))
      .map((file) => file.substring(1))
      .map((file) => ({ from: file, to: file.replace('_failed', '') }))
      .forEach(({ from, to }) => {
        fs.copySync(
          path.resolve(RUNS_FOLDER_PATH, lastRun, from),
          path.resolve(REFERENCES_FOLDER_PATH, to),
        );
      });
    ...
    for (const report of results) {
      ...
      for (const step of steps) {
        ...
        for (let viewport of viewports) {
          ...
          const original = path.resolve(REFERENCES_FOLDER_PATH, report.scenarioFolder, viewport.name, `step${step.stepNumber}.png`);
          const target = path.resolve(RUNS_FOLDER_PATH, report.timestamp, report.scenarioFolder, viewport.name, `step${step.stepNumber}.png`);
